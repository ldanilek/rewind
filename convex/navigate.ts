...
export default mutation(
  async ({ db, auth, scheduler }, { operation }: { operation: Operation }) => {
    ...
    if (
      (operation === Operation.Rewind ||
        operation === Operation.UseTurnstile) &&
      config.maxRewinds === game.currentPlayerIndex
    ) {
      console.log("no more rewinds");
      return;
    }
    const state = await getGameState(db, game, scheduler);
    if (state === null) {
      return;
    }
    ...
    const playerObject =
      gameState.objects[config.initialObjects.length + game.currentPlayerIndex];
    ...
    if (operation === Operation.Rewind) {
      ...
      for (let object of gameState.objects) {
        if (
          object.objectType === GameObjectType.Turnstile &&
          object.locationX === playerObject.locationX &&
          object.locationY === playerObject.locationY
        ) {
          onTurnstile = true;
          break;
        }
      ...
      if (onTurnstile) {
        const newPlayerIndex = game.currentPlayerIndex + 1;
        const newTimeFlow =
          game.timeFlow === TimeFlow.Forward
            ? TimeFlow.Backward
            : TimeFlow.Forward;
    ...
    // Schedule next move.
    const updatedGame = (await getGame(db, user))!;
    await getGameState(db, updatedGame, scheduler);
  }
);
